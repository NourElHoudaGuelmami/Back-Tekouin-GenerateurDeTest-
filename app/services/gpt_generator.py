...
def generate_certification_test_from_course22(course_text: str, course_objectives: dict, seniority_input: str, num_questions: int, language_input: str) -> dict:
...
============================================================
🎓 UNIVERSITY-LEVEL REASONING ENFORCEMENT
============================================================

Each question MUST reflect how a university professor evaluates students.

MANDATORY CHARACTERISTICS OF A VALID QUESTION:

1. The question MUST involve a decision, evaluation, or choice
   that requires weighing at least TWO plausible options.

2. Wrong choices MUST be:
   - Technically plausible
   - Logically consistent
   - Directly derived from the course content
   - Incorrect ONLY because of a precise rule or condition

3. The correct answer MUST NOT be obvious by elimination
   without applying a specific rule from the raw_excerpt.

4. Explanations MUST:
   - Explicitly reference the decisive rule or condition
   - Explain WHY the other options are incorrect
   - Use an academic, neutral, precise tone (no marketing, no simplification)

If a question can be answered by “general IT knowledge”
without studying the course, it is INVALID and MUST be regenerated.
...
--------------------------------------------------
============================================================
🎯 SENIORITY-LEVEL COGNITIVE CONTRACT (NON-NEGOTIABLE)
============================================================

You MUST strictly adapt question complexity to the learner level.
...
- Can apply rules in DIRECT, UNAMBIGUOUS situations

Question constraints:
- Single rule applied
- Explicit context
- No conflicting conditions
- Focus on correct classification or consequence
...
------------------------------------------------------------
Target profile:
- Has practical exposure
- Can reason across multiple concepts

Question constraints:
- At least TWO interacting rules or constraints
- Context with a decision boundary
- Require comparison or prioritization

INVALID for Mid-Level:
- Pure recall
- Single-rule questions

------------------------------------------------------------
EXPERT
------------------------------------------------------------
Target profile:
- Architect / senior engineer / technical lead

Question constraints:
- Complex scenario
- Implicit trade-offs
- Incomplete information
- Require judgment, evaluation, or justification

Each Expert question MUST:
- Involve risk, performance, scalability, or fault-tolerance trade-offs
- Require explaining WHY a choice is better, not just WHICH one

------------------------------------------------------------
If a question does not match its seniority contract, it is INVALID.
...
1. Create EXACTLY ONE category per learning objective.
2. Each category MUST:
   - Clearly correspond to the objective intent
   - Be a SHORT, PROFESSIONAL, EXAM-READY label
   - NOT be a copy or trivial paraphrase of the objective text
   - Represent a KNOWLEDGE / SKILL AREA, not a sentence

3. Categories MUST be output in a JSON object named "categories_map"
   

RULES (STRICT):
- One and only one category per objective
- No additional categories allowed
- These category labels MUST be reused verbatim for all questions
- If a category is poorly defined, the exam is INVALID

ONLY AFTER defining categories_map may you generate questions.
